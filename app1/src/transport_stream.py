...
def parsePMTSection(fileHandle, k):
    ...
    while n > 0:
        ...
        logging.debug(
            "descriptor_tag = %d, descriptor_length = %d"
    ...
    while length > 0:
        ...
        logging.debug(
            "stream_type = 0x%X, elementary_PID = 0x%X, ES_info_length = %d"
...
def parseSITSection(fileHandle, k):
    ...
    while length > 0:
        ...
        logging.debug(
            "service_id = %d, service_loop_length = %d"
            % (service_id, service_loop_length)
        )
...
def getDeltaStats(listDelta):
    ...
    return {
        "min": int(minVal),
        "max": int(maxVal),
        "average": int(total / len(listDelta)),
    }
...
def getTrackStat(pid, count, pts):
    ...
    return {
        ...
        "size": int(size),
        "bandwidth": int((8 * 1000 * size) / duration),
    }
...
def parsePcrPts(fileHandle):
    ...
    try:
        while True:
            ...
            if syncByte != 0x47:
                logging.error(
                    "Ooops! Can NOT found Sync_Byte! maybe something wrong with the file"
                )
                break
            ...
            if (adaptation_fieldc_trl == 0x2) | (adaptation_fieldc_trl == 0x3):
                [Adaptation_Field_Length, flags] = parseAdaptation_Field(
                    fileHandle, n + 4, PCR
                )
                ...
                if (flags >> 4) & 0x1:
                    ...
                    logging.debug(
                        "PCR packet, packet No. %d, PID = 0x%x, PCR = 0x%X discontinuity = %s"
            ...
            if (adaptation_fieldc_trl == 0x1) | (adaptation_fieldc_trl == 0x3):
                ...
                if (PESstartCode & 0xFFFFFF00) == 0x00000100:
                    ...
                    if payload_unit_start_indicator == 1:
                        parsePESHeader(
                            fileHandle, n + Adaptation_Field_Length + 4, PESPktInfo
                        )
                        logging.debug(
                            "PES start, packet No. %d, PID = 0x%x, PTS = 0x%X"
                            % (packetCount, PID, PESPktInfo.PTS)
                        )
                        PTSList.append(
                            {"packet": packetCount, "pid": PID, "pts": PESPktInfo.PTS}
                        )
                    ...
                    table_id = readFile(
                        fileHandle,
                        n + Adaptation_Field_Length + 4 + 1 + pointer_field,
                        1,
                    )

                    if (table_id == 0x0) & (PID != 0x0):
                        logging.warning(
                            "Ooops!, Something wrong in packet No. %d" % packetCount
                        )
                    ...
                    if table_id == 0x0:
                        logging.debug(
                            "pasing PAT Packet! packet No. %d, PID = 0x%X"
                            % (packetCount, PID)
                        )
                        ...
                        logging.debug(
                            "pasing PMT Packet! packet No. %d, PID = 0x%X"
                            % (packetCount, PID)
                        )
                        ...
                        logging.debug(
                            "pasing SIT Packet! packet No. %d, PID = 0x%X"
                            % (packetCount, PID)
                        )
