...
def sink_aggregation(json_data):
    ...
    aggregate_data["wifiAggregate"]["deviceType"] = json_data["wifiData"][0][
        "deviceType"
    ]
    aggregate_data["wifiAggregate"]["countBandChange"] = count_value_change(
        json_data["wifiData"], "connection"
    )
    ...
    aggregate_data["wifiAggregate"]["avgRSSI"] = calculate_avg(
        json_data["wifiData"], "rssi"
    )
    aggregate_data["anomalies_report"] = detect_anomaly_min(
        json_data["wifiData"], "rssi", RSSITHRESHOLD
    )
